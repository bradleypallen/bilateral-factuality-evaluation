...
#         t1 = perf_counter()
#         evaluation_responses = [ self.verify_chain.invoke(datapoint) for i in range(samples) ]
#         t2 = perf_counter()
...
class BilateralJudge(Model):
    ...
    def __init__(self, model_name, prompt_type="baseline", temperature=0.0):
        super().__init__(model_name, temperature)
        if prompt_type == "baseline":
            verification_prompt = prompts.BASELINE_VERIFICATION_PROMPT
            refutation_prompt = prompts.BASELINE_REFUTATION_PROMPT
        elif prompt_type == "zero":
